...
class EditUser extends EditController
{
    ...
    protected function editAction(): bool
    {
        ...
        if ($result && $this->tab('EditUser')->model->nick === $this->user->nick) {
            $newLang = $this->tab('EditUser')->model->langcode;
            if ($this->user->langcode !== $newLang) {
                Tools::lang()->setLang($newLang);
                
                $expire = time() + Tools::config('cookies_expire');
                $this->response->cookie('fsLang', $newLang, $expire);
                
                // Redirigir para forzar la recarga de la interfaz con el nuevo idioma
                $this->redirect($this->url(), 0);
            }
        }
