...
class Installer implements ControllerInterface
{
    ...
    {
        $this->db_host = strtolower(trim($this->request->input('fs_db_host', getenv('DB_HOST') ?: 'localhost')));
        $this->db_name = trim($this->request->input('fs_db_name', getenv('DB_DATABASE') ?: 'facturascripts'));
        $this->db_pass = $this->request->input('fs_db_pass', getenv('DB_PASSWORD') ?: '');
        $this->db_port = (int)$this->request->input('fs_db_port', getenv('DB_PORT') ?: 3306);
        $this->db_type = $this->request->input('fs_db_type', getenv('DB_CONNECTION') ?: 'mysql');
        $this->db_user = trim($this->request->input('fs_db_user', getenv('DB_USERNAME') ?: 'root'));
