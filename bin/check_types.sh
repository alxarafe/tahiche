...
if docker ps --format '{{.Names}}' | grep -q "^tahiche_php$"; then
    ...
    docker exec -e PHPSTAN_RUNNING=1 tahiche_php php -d error_reporting="E_ALL & ~E_DEPRECATED" ./vendor/bin/phpstan analyse -c phpstan.neon --memory-limit=1G --no-progress
elif [ -f "$PROJECT_DIR/vendor/bin/phpstan" ]; then
    echo "  Ejecutando PHPStan localmente..."
    PHPSTAN_RUNNING=1 php -d error_reporting="E_ALL & ~E_DEPRECATED" "$PROJECT_DIR/vendor/bin/phpstan" analyse -c "$PROJECT_DIR/phpstan.neon" --memory-limit=1G --no-progress
else
